...
Formalising parts of [this math.SE answer](https://math.stackexchange.com/a/4943344), with
finite-dimensional real vector spaces replaced with finite sets;
the situation is entirely analogous.

Instead of the skeletal category whose objects are natural numbers
representing $ℝⁿ$ and whose morphisms are linear maps, we use the skeletal
...
Instead of the univalent category of finite-dimensional real vector
spaces, we use the univalent category of finite sets, here realised as
the *essential image* $C$ of the inclusion of $S$ into sets.
...
If we remove the truncation (but do not change the morphisms),
...
of the image of a function using $Σ$ instead of $∃$
[yields](https://1lab.dev/1Lab.Counterexamples.Sigma.html) the domain of the function.
...
[global choice](https://1lab.dev/1Lab.Counterexamples.GlobalChoice.html),
which is just false in homotopy type theory.
